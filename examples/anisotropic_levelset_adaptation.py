#!/usr/bin/env python3
"""
Anisotropic Level-Set Adaptation Example

A level-set based anisotropic remeshing implementation showing:
1. Metric field construction from a level-set function (sine curve)
2. Split/collapse operations based on metric edge length
3. Laplacian smoothing in physical space
4. Comprehensive visualization

This example demonstrates adaptive anisotropic remeshing where:
- The metric field is defined by distance to a level-set (\phi = y - sin(2\pi x))
- Fine resolution perpendicular to the iso-contour (curve)
- Coarse resolution tangent to the iso-contour
- Triangle elongation aligns with the level-set geometry
"""
...
def sine_curve_levelset(x, y):
    """Level-set function: \phi(x,y) = y - (0.5 + 0.15·sin(4\pi x))"""
    amplitude = 0.15
    frequency = 4.0 * np.pi
...
def plot_mesh(editor, metric_fn, initial_boundary_verts=None, initial_mesh=None, 
    ...
    ax3.set_title('Zoom: Alignment with Curve\n(x in [0.25, 0.55], y in [0.35, 0.65])', fontsize=11, fontweight='bold')
