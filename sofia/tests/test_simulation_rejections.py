...
def test_simulation_rejects_boundary_loop_increase():
    ...
    ok, msgs, inv = editor._simulate_compaction_and_check(cand_pts, cand_tris_sim)
    # Should reject (either due to loop increase or inverted geometry). Current implementation flagged an inversion.
    assert not ok, f"Expected rejection but got ok. msgs={msgs} inv={inv}"
    if not any('loop' in m for m in msgs):
        # fallback: ensure an inverted triangle was detected
        assert inv, "Expected inverted triangle list when no loop message present"
...
def test_simulation_rejects_inverted_triangle():
    ...
    ok, msgs, inv = editor._simulate_compaction_and_check(cand_pts, cand_tris)
    assert not ok, "Inverted triangle should trigger rejection"
    assert len(inv) > 0, "Inverted list should report at least one triangle"
...
def test_simulation_accepts_clean_extension():
    ...
    ok, msgs, inv = editor._simulate_compaction_and_check(cand_pts, cand_tris)
    assert ok, f"Clean extension should pass but failed: msgs={msgs} inv={inv}"
    assert len(inv) == 0
...
def test_simulation_rejects_crossing_edges():
    ...
    ok, msgs, inv = editor._simulate_compaction_and_check(cand_pts, cand_tris)
    assert not ok, f"Expected crossing-edges rejection, got ok. msgs={msgs} inv={inv}"
