...
\item \textbf{Full Pipeline}: Refine → Optimize → Coarsen → Final optimization (multi-stage)
\end{itemize}
...
\midrule
Square   & 42  & 57  & 1.36× & 48.2 & 0.05 \\
Circle   & 162 & 241 & 1.49× & 45.7 & 0.18 \\
L-shape  & 94  & 176 & 1.87× & 42.9 & 0.11 \\
Airfoil  & 384 & 627 & 1.63× & 44.8 & 0.38 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Observations:}
\begin{itemize}
\item Refinement ratio varies by geometry: L-shape requires most refinement (1.87×) due to large initial triangles near corners, while Square is nearly uniform (1.36×).
...
After 3 iterations, maximum boundary edge length reduced by 62.4\% (from 0.245 to 0.092), improving boundary approximation error from $O(h^2) \approx 0.06$ to $O(h^2) \approx 0.008$ (7.5× reduction in geometric error). Quality simultaneously improves due to better aspect ratios.
...
\item \textbf{Area-based refinement} ($A_{\max} = 0.008$): 203 triangles, $Q_{\min} = 40.8°$
  ...
  \item Slight quality decrease (42.4° → 40.8°) due to new triangles, but still excellent
  \end{itemize}
\item \textbf{Final optimization} (5 iterations): 203 triangles, $Q_{\min} = 41.7°$
  \begin{itemize}
  \item Recovers quality after refinement (40.8° → 41.7°)
...
\end{itemize}
  	...
  	extbf{Example:} Heat diffusion on L-shape with singularity at reentrant corner. Using a recovery-based indicator (Z--Z), adaptive refinement reduces error from 12.3\% to 1.8\% using 2.5× fewer elements than uniform refinement.
...
\begin{itemize}
\item \textbf{Mesh statistics}: 1894 triangles, 980 vertices (11.7× increase)
\item \textbf{Metric edge lengths}: $L_M \in [0.29, 1.20]$ with mean $0.96$ (target is 1.0)
  ...
  \end{itemize}
\item \textbf{Approximation error}: Reduced from $0.500$ to $0.164$ (3.05× improvement)
  \begin{itemize}
...
\begin{itemize}
\item Estimated $\sim 15{,}000$ triangles (vs. 1894 anisotropic, 7.9× increase)
\item $\sim 7{,}500$ vertices (vs. 980, 7.7× increase)
