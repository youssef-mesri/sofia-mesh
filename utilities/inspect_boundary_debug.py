...
from sofia.sofia.patch_batching import vertex_patch
...
def main():  # pragma: no cover
    ...
    def walk_oriented(a0,b0):
        ...
        while True:
            ...
            print('\tcurr', curr, 'prev', prev, 'neighs', neighs, 'candidates', candidates)
            if not candidates:
                if loop[0] in neighs:
                    loop.append(loop[0]); print('\tclosing to start');
                break
            ...
            for c in candidates:
                ...
                print('\tcandidate', c, 'ang_out', ang_out, 'd', d)
                if best is None or d < best_ang:
                    best = int(c); best_ang = d
            nxt = best
            ekey = tuple(sorted((int(curr), int(nxt))))
            print('\tnext', nxt, 'edgekey', ekey, 'visited?', ekey in visited_edges)
            if ekey in visited_edges:
                if nxt == loop[0]: loop.append(loop[0]); print('\tclosing by visited edge');
                break
            loop.append(int(nxt)); visited_edges.add(ekey)
            prev, curr = curr, nxt
            safety += 1
            if safety > 1000:
                print('safety break'); break
    ...
    print('poly', p)
    print('visited_edges after walk', visited_edges)
    return 0
