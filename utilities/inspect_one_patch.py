...
logger.info('candidate nodes (by valence): %s', nodes[:10])
seed = nodes[0]
logger.info('seed: %s', seed)
tris_set = set(vertex_patch(editor, seed, radius=1))
logger.info('vertex_patch tris: %s', tris_set)
...
logger.info('boundary_edges via edge_map: %s', boundary_edges_emap)
...
logger.info('boundary_edges via recount: %s', boundary_edges_count)

polys = patch_boundary_loops(tris_set, editor)
logger.info('patch_boundary_loops returned: %s', polys)
...
for t in sorted(tris_set):
    ...
    logger.info('tri %s', coords.tolist())
